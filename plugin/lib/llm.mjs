...
async function detectProvider() {
  ...
  if (_userConfig?.llmApiKey) {
    const base = (_userConfig.llmEndpoint || "https://api.openai.com/v1").replace(/\/+$/, "");
    const model = _userConfig.llmModel || "gpt-4o-mini";
  ...
  // 4. Env var fallback
  const envKey = process.env.OPENCLAW_LLM_API_KEY || process.env.OPENAI_API_KEY;
  if (envKey) {
