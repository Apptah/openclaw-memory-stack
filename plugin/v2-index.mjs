...
const HYDE_MODEL = process.env.HYDE_MODEL || "qwen2.5:7b";
const HYDE_ENDPOINT = process.env.HYDE_ENDPOINT || "http://localhost:11434";

function hydeExpand(query) {
  if (!HYDE_ENABLED) return null;
  try {
    const prompt = `Write a short technical passage (~150 words) that directly answers this developer question. Be specific with code terms and patterns:\n\nQuestion: ${query}`;
    const payload = JSON.stringify({
      model: HYDE_MODEL, prompt, stream: false,
      options: { temperature: 0.7, num_predict: 300 },
    });
    const result = execSync(
      `curl -s --max-time 5 -X POST "${HYDE_ENDPOINT}/api/generate" -H "Content-Type: application/json" -d '${payload.replace(/'/g, "'\\''")}'`,
...
function tierContent(content, tier) {
  if (!content || tier === "L2") return content;
  if (tier === "L0") {
    // ~100 tokens: first meaningful sentence
    const sentences = content.split(/[.!?\n]/).filter(s => s.trim().length > 20);
    return (sentences[0] || content.slice(0, 400)).trim();
  }
  // L1: ~2K tokens (~8000 chars)
  if (content.length <= 8000) return content;
  return content.slice(0, 8000) + "\n[... truncated, use L2 for full content]";
}

function autoSelectTier(relevance, resultCount) {
  // High relevance + few results → load more detail
  if (relevance > 0.7 && resultCount <= 3) return "L1";
  if (relevance > 0.9) return "L2";
  return "L0";
}
...
const QMD_BIN = (() => {
  // Find qmd binary
  const paths = [
    resolve(HOME, ".claude/plugins/cache/qmd/qmd/0.1.0/qmd"),
...
function combinedSearch(query, maxResults, maxTokens, searchMode) {
  ...
  // RRF (Reciprocal Rank Fusion) — merge by rank position, not raw scores
  const K = 60; // RRF constant
  const sources = [
    { results: rescue, weight: 1.0 },
    { results: graphResults, weight: 1.2 },
    { results: sessions, weight: 0.8 },
    { results: qmd, weight: 1.0 },
    { results: fts5, weight: 1.0 },
    { results: md, weight: 0.7 },
  ];
  const rrfScores = new Map(); // key -> { score, item }
  for (const { results: srcResults, weight } of sources) {
    for (let rank = 0; rank < srcResults.length; rank++) {
      const r = srcResults[rank];
      const key = (r.content || "").slice(0, 80).toLowerCase();
      const rrfScore = weight / (K + rank + 1);
      const existing = rrfScores.get(key);
      if (existing) {
        existing.score += rrfScore;
        // Keep the higher-detail version
        if ((r.content || "").length > (existing.item.content || "").length) existing.item = r;
      } else {
        rrfScores.set(key, { score: rrfScore, item: r });
      }
    }
  }
  const all = [...rrfScores.values()]
    .sort((a, b) => b.score - a.score)
    .map(v => { v.item.relevance = Math.min(1, v.score * 50); return v.item; });

  // Already deduped by RRF key merging — just trim to maxResults
  const deduped = all.slice(0, maxResults * 2); // Keep extra for MMR to work with
  ...
  // Tiered loading: auto-select detail level based on relevance + count
  const tier = autoSelectTier(
    reranked[0]?.relevance || 0,
    reranked.length
  );
  const tiered = reranked.map(r => ({
    ...r,
    content: tierContent(r.content, tier),
    tier,
  }));
...
function applyTemporalDecay(results, halfLifeDays = 30) {
  ...
  // Bi-temporal: look for event_time in content frontmatter
  const eventTimePattern = /event_time:\s*(\d{4}-\d{2}-\d{2}[T\s]?\d{2}:\d{2})/;

  for (const r of results) {
    // Evergreen sources are exempt
    if (r.source === "MEMORY.md" || (r.source || "").startsWith("rescue:")) continue;

    // Bi-temporal: prefer event_time over record_time (source date)
    let docDate = null;
    const eventMatch = (r.content || "").match(eventTimePattern);
    if (eventMatch) {
      docDate = new Date(eventMatch[1]).getTime();
    }
    if (!docDate || isNaN(docDate)) {
      const match = (r.source || "").match(datePattern);
      if (!match) continue;
      docDate = new Date(match[0]).getTime();
    }
